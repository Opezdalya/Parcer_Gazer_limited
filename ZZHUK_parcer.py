...
def write_to_csv_clear(items) -> None:
    ...
    sheet_instance = sheet.get_worksheet(14)
...
def write_to_csv(items) -> None:
    ...
    sheet_instance = sheet.get_worksheet(36)
...
def parse(url_search: str) -> List[Tuple[str, str, str]]:
    ...
    try:
        ...
        while page <= last_page:
            url = url_search
            if page > 1:
                url = f'https://zhuk.ua/catalog/search/filter/brand=542;page={page}/?q=Gazer'

            print(f'Load: {url}')
            driver.get(url)

            for item_el in driver.find_elements_by_css_selector(".catalogCard-info"):
                name = get_text_by_css(item_el, '.catalogCard-title', 'Null')
                price = get_text_by_css(item_el, '.catalogCard-price', '-')
                nal = get_text_by_css(item_el, '.catalogCard-availability', '-')
...
if __name__ == '__main__':
    url = "https://zhuk.ua/catalog/search/filter/brand=542;page=1/?q=Gazer"
    items = parse(url)
    print(f'Total goods: {len(items)}')

    file_name = f'ZZHUK_parser_{DT.datetime.now():%Y-%m-%d}.csv'
